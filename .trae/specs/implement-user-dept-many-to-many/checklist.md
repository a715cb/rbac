...
- [x] 权限计算单元测试覆盖单部门、多部门、主部门变更、各 data_scope 场景（9 个新增测试，28 tests/36 assertions 全部通过）
...
## 阶段4：数据迁移与验证
- [ ] 数据迁移脚本在测试环境正确执行（需备库后执行 `check_user_dept_consistency.php`）
- [x] 一致性校验脚本 `check_user_dept_consistency.php` 已就绪（4 项检查：孤立记录、主部门一致性、多主部门冲突、无效部门引用）
- [ ] 用户部门关系查询响应时间 < 200ms（需 Apache Bench 压测，索引已到位）
- [ ] 批量操作处理能力 > 1000条/分钟（需数据库有足够数据量验证）
- [ ] 历史功能在新系统中保持原有行为（需人工回归测试）
- [ ] 权限控制逻辑无安全漏洞（需人工安全审计）
- [x] 回滚方案已制定（代码回滚+数据回滚+前端恢复+验证 4 步）
