<!--
  @文件: TagsView/index.vue
  @用途: 多标签页视图组件
  @描述: 展示已访问页面的标签页，支持点击切换、关闭当前、关闭其他、关闭全部等操作。
         与 useTabs Hook 配合使用，管理标签页的列表、激活状态和操作逻辑。
         支持固定定位（fixedMultiTabs）和多种标签页样式（smooth-tab/smart-tab）。
  @核心逻辑:
    1. 使用 useTabs 管理标签页状态（列表、激活项）和操作方法（关闭、跳转）
    2. 固定标签页时使用 fixed 定位，通过占位元素避免内容遮挡
    3. 标签页切换时保留原始 query 参数，确保页面数据正确加载
    4. 右侧下拉菜单提供批量关闭操作
-->
<template>
  <!--
