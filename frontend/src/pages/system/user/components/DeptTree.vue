<!--
  ...
  ─────────────────────────────────────────────
  功能描述：
    在用户管理页面左侧展示部门树形结构，支持搜索过滤、展开/折叠全部节点，
    选中部门后向父组件发送选中事件，用于按部门筛选用户列表。

  核心交互：
    1. 页面加载时自动获取部门树数据并默认展开全部节点
    2. 点击部门节点 → 向父组件 emit('select', deptId) → 触发用户列表按部门筛选
    3. 搜索部门 → 递归过滤树节点，自动展开匹配节点
    4. 展开全部 / 折叠全部 → 快捷操作树节点展开状态

  数据流说明：
    getDeptTree() → treeData(原始数据) → filteredTreeData(搜索过滤) → a-tree 渲染
    用户选中 → handleSelect → emit('select', deptId) → 父组件按部门筛选用户

  依赖接口：
    - getDeptTree()：获取部门树形数据

  依赖类型：
    - DeptInfo：部门节点数据结构
        { id: number, name: string, children?: DeptInfo[] }

  对外事件：
    - select(deptId: number | undefined)：选中部门时触发，未选中时传 undefined

  暴露方法：
    - resetSelection()：重置选中状态（父组件可通过 ref 调用）

  注意事项：
    - default-expand-all 仅在首次渲染生效，后续展开状态完全由 expandedKeys 控制
    - 搜索过滤采用"保留祖先链"策略：即使父节点不匹配，只要子树有匹配项也会保留
    - 选中模式为单选，点击已选中节点可取消选中
-->
<template>
