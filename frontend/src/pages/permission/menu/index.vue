...
const statusChangingIds = ref<Set<number>>(new Set()) // 正在状态变更中的菜单ID集合，防止重复点击
 ...
 * @description 先更新本地状态实现即时反馈，API 失败时回滚状态，防止重复点击
 */
const handleStatusChange = async (record: MenuInfo, checked: boolean) => {
  // 防止重复点击：如果该菜单正在变更中，直接返回
  if (statusChangingIds.value.has(record.id)) return

  statusChangingIds.value.add(record.id)
  const oldStatus = record.status
  record.status = checked ? 1 : 0

  try {
    await changeMenuStatus(record.id, checked ? 1 : 0)
    ...
    statusChangingIds.value.delete(record.id)
  }
}
